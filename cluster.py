...
class KMeans:
    ...
    def _nearest_centroid(self, sample, centroids):
        similarities = [Cosine_Sim(sample, c) for c in centroids]
        #print(similarities)
        nearest_centroid = np.argmax(similarities)
    ...
    """def _is_converged(self, old_centroids, new_centroids):
        dist = [Cosine_Sim(old_centroids[i], new_centroids[i]) for i in range(self.K)]
    ...
    def predict(self, X):
        ...
        #print("\nChosen centroids are:")
        #for cin in self.centroids:
         #   print(cin)
    ...
    def plot_data(self):
        ...
        for i,index in enumerate(self.clusters):            
            ...
            #ax.scatter(*cen,c=labels_color_map[i],marker="x",linewidth=2) 
